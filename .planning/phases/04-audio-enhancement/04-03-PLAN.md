...
must_haves:
  ...
  artifacts:
    - path: "src/hooks/useTTS.ts"
      ...
      updates: "Added speaker parameter, integrated with audio store"
    - path: "src/components/StreamingTranscriptEntry.tsx"
      provides: "Streaming transcript with integrated TTS playback"
      updates: "Added TTS integration, voice selection per speaker role"
...
<task type="auto">
  <name>Task 1: Enhance useStreamingTTS hook with role-based voice and audio store integration</name>
...
6. Update speakText function to accept speaker parameter and use voice registry

DO NOT:
- Add voice selection UI (that's in AudioControls component)
- Change audio playback mechanism (handled by AudioControls component)
...
Build succeeds without TypeScript errors.
  ...
  <done>
useStreamingTTS hook enhanced with speaker parameter, uses getVoiceIdForRole for voice selection, checks isEnabled from audioStore before generating TTS.
  </done>
</task>
...
2. In component props (accessed via useDebateStore):
   - Get streamingEntry object which contains speaker role
3. Initialize useStreamingTTS hook:
   - Call with { speaker: streamingEntry.speaker, enabled: true, bufferSize: 150 }
...
- Create new audio elements (use hidden element from AudioControls)
...
Build succeeds without TypeScript errors.
  ...
  <done>
StreamingTranscriptEntry integrated with useStreamingTTS, queues text to TTS as it streams, flushes on completion, respects speaker role for voice selection.
  </done>
</task>
